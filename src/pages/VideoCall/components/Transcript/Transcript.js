...
import useSymblContext from "../../hooks/useSymblContext";
...
export function TranscriptElement({
  onSave,
  width,
  height,
  editable = false,
      ...
      <Draggable>
        ...
        >
          ...
          <Grid
            ...
            ref={containerRef}
            style={{ height: `calc(${h} - 62px)` }}
          >
            {transcriptItems
              .filter((item) => !!item)
              .map(({ text, timeDiff, from }, index) => (
                <TranscriptItem
                  key={index}
                  index={index}
                  description={text}
...
export default function Transcript({ height }) {
  ...
  useEffect(() => {
    if (newMessages && newMessages.length > 0) {
      const newTranscriptItems = newMessages.map((message) =>
        convertMessageToTranscriptItem(message, startedTime)
      );
  ...
  return (
    <TranscriptElement transcriptItems={transcriptItems} height={height} />
  );
}
